...
async function scrapeHtmlByLabelAndGenerateJSON() {
  ...
  try {
    ...
    let dataWithLabel = dataHtmlWithAccordion
      .concat(dataPublicServices)
      .concat(dataPermisosEne24);
  ...
  try {
    ...
    for (const url of urls) {
      ...
      const label = document
        .querySelector('.box.box-warning p')
...
async function generateHtmlWithAccordionDocuments() {
  const urlsAccordion = [
    'https://www.monterrey.gob.mx/transparencia/Oficial/SAYUNTAMIENTO.asp',
  ...
  for (const url of urlsAccordion) {
    ...
    document.querySelectorAll('.accordion').forEach((element) => {
      ...
      const documentos = [];
      const elements =
        element.parentElement.querySelector('.panel3') ||
        element.parentElement.querySelector('.panel33');
